...
This prototype has positional and explicit entity-key identity. Build-generated view-function identity is not integrated with the Bun entry yet. Same-kind unkeyed branches reuse native state, so it does not yet provide Foldkit's browser branch-reset semantics. It also has no terminal Mount API, Submodel Message mapping helper, lazy views, or widgets beyond boxes, text, inputs, and mouse-activated buttons. The package stays private while these boundaries are developed.
